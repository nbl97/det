...
class ZipReader(object):
    ...
    def read(self, zip_file, image_name):
        if zip_file in self.id_context:
            with self.id_context[zip_file].open(image_name) as f:
                tmp = f.read()
            return tmp
        else:
            file_handle = zipfile.ZipFile(zip_file, 'r', zipfile.ZIP_LZMA)
            self.id_context[zip_file] = file_handle
            return self.id_context[zip_file].read(image_name)
...
class LoadImageFromZip(object):
    ...
    def __call__(self, results):
        ...
        img = cv2.imdecode(np.frombuffer(tmp, np.uint8), 1)
...
class LoadAnnotations(object):
    ...
    def _load_semantic_seg(self, results):
        ...
        zip_path = 'data/coco/stuffthingmaps_trainval2017.zip'

        filename = osp.join('train2017', results['ann_info']['seg_map'])

        tmp = self.zipreader.read(zip_path, filename)
        img = cv2.imdecode(np.frombuffer(tmp, np.uint8), cv2.IMREAD_UNCHANGED).squeeze()
